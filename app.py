...
# --- 1. 인증 (코드에 키 없음! 오직 Secrets만 확인) ---
def get_creds():
    # Streamlit Secrets에서 설정을 찾습니다.
    if "gcp_service_account" not in st.secrets:
        st.error("🚨 **인증 정보가 없습니다!**")
        st.info("이 코드는 키를 포함하지 않습니다. Streamlit 홈페이지 [Settings] > [Secrets] 메뉴에 키를 넣어주세요.")
        st.stop()

    secrets_data = st.secrets["gcp_service_account"]

    # 1) 'info'에 JSON 전체를 붙여넣은 경우 (추천 방식)
    if "info" in secrets_data:
        try:
            return json.loads(secrets_data["info"], strict=False)
        except json.JSONDecodeError:
            st.error("🚨 Secrets의 'info' 내용이 올바른 JSON 형식이 아닙니다.")
            st.stop()
    
    # 2) type, project_id 등을 각각 넣은 경우
