...
def get_creds():
    # Streamlit Secrets에서 설정을 찾습니다.
    if "gcp_service_account" not in st.secrets:
        st.error("🚨 **비밀 금고(Secrets)가 비어있습니다!**")
        st.info("Streamlit 홈페이지 > 앱 설정(Settings) > Secrets 에 키 내용을 넣어주세요.")
        st.stop()

    secrets_data = st.secrets["gcp_service_account"]

    # 'info'라는 이름으로 JSON을 통째로 넣은 경우
...
with st.sidebar:
    ...
    st.divider(); st.subheader("🎯 목표치 설정")
    ...
    if st.button("💾 설정값 영구 저장"):
        new_conf = {
            ...
            "t_mg":t_mg, "t_no2":t_no2, "t_no3":t_no3, "t_po4":t_po4, "t_ph":t_ph,
...
with st.expander("📝 새 기록 입력하기", expanded=False):
    with st.form("entry"):
        ...
        d_ca=c2.number_input("Ca",value=int(cfg["t_ca"]),step=10)
        d_mg=c2.number_input("Mg",value=int(cfg["t_mg"]),step=10)
        d_no2=c3.number_input("NO2",value=0.0,format="%.3f",step=0.001)
        d_no3=c3.number_input("NO3",value=float(cfg["t_no3"]),step=0.1)
        d_po4=c3.number_input("PO4",value=float(cfg["t_po4"]),format="%.3f",step=0.01)
        d_ph=c4.number_input("pH",value=float(cfg["t_ph"]),step=0.1)
        d_sal=c4.number_input("염도",value=35.0,step=0.1)
        d_temp=c4.number_input("온도",value=25.0,step=0.1)
        d_memo=st.text_area("메모 (특이사항 등)")
...
if not df.empty:
    ...
    with g1:
        ...
        col_g1.plotly_chart(draw_radar(["KH","Ca","Mg"],[last["KH"],last["Ca"],last["Mg"]],[cfg["t_kh"],cfg["t_ca"],cfg["t_mg"]],"3요소","#00FFAA"), use_container_width=True)
        col_g2.plotly_chart(draw_radar(["NO2","NO3","PO4","pH"],[last["NO2"],last["NO3"],last["PO4"]*100,last["pH"]],[cfg["t_no2"],cfg["t_no3"],cfg["t_po4"]*100,cfg["t_ph"]],"영양염","#FF5500"), use_container_width=True)
    ...
    with g2:
        ...
        new_sch = st.text_area("주간 계획", value=current_sch, height=150, placeholder="예: 월요일 환수")
        if st.button("💾 스케줄 저장"):
